...
class Generator(nn.Module):
 ...
 def forward(self,x):
   ...
   d1 = torch.cat([F.dropout(self.d1(e8_bis), 0.5, training=True), e7], 1)
   d1_bis = F.relu(d1)

   d2 = torch.cat([F.dropout(self.d2(d1_bis), 0.5, training=True), e6], 1)
   d2_bis = F.relu(d2)

   d3 = torch.cat([F.dropout(self.d3(d2_bis), 0.5, training=True), e5], 1)
   d3_bis = F.relu(d3)

   d4 = torch.cat([self.d4(d3_bis), e4], 1)
   d4_bis = F.relu(d4)

   d5 = torch.cat([self.d5(d4_bis), e3], 1)
   d5_bis = F.relu(d5)

   d6 = torch.cat([self.d6(d5_bis), e2], 1)
   d6_bis = F.relu(d6)

   d7 = torch.cat([self.d7(d6_bis), e1], 1)
   d7_bis = F.relu(d7)
